"""
NiceGUI web app for the four-room fruit-collection experiment.
...
async def start_experiment(meta_container, stage_container):
    ...
    if "gw_state" not in app.storage.user:
        ...
        state = app.storage.user["gw_state"]
        logger.info(
            f"New episode: apple={state['fruit_values']['apple']}pts, "
            f"banana={state['fruit_values']['banana']}pts"
        )
...
async def finish_experiment(container):
    ...
    metadata = {
        ...
        "fruit_values":    state.get("fruit_values"),
        "fruit_positions": state.get("fruit_positions"),
        "fruit_types":     state.get("fruit_types"),
        "grid":            state.get("grid"),
...
ui.run(
    storage_secret="fourroom_fruits_42",
    show=False,
    reload=True,
